...
?>
<center><font size="2">
  <a href="<?=$path->getPath("page_gameviz_fullscreen")?>?driveid=<?=$driveid?>&gameid=<?=$gameid?>">Full Screen</a>
  &nbsp;&nbsp;<u>Drive: <?=$homeaway?><?=$driveindex?></u>&nbsp;&nbsp;
  <a href="<?=$p->pageName()?>?gameid=<?=$gameid?>">Current</a><br>
  <a href="<?=$p->pageName()?>?gameid=<?=$gameid?>&last15=true">Last 15 Plays</a>
</center><br>

<img src="<?=$path->getPath("page_gameviz_image")?>?driveid=<?=$driveid?>&gameid=<?=$gameid?><?=$last15?>&nocache=<?=uniqid("viz_image_", 1)?>"><br>

<center><font size="2"><u>Choose another drive:</u></font><br>
  ...
  foreach ($drives as $d) {
    if ($d[quarter] == $q) {
      ...
      if ($d[driveid] != $driveid) {?>
        <a style="color:<?=$d[color]?>" href="<?=$p->pageName()?>?driveid=<?=$d[driveid]?>&gameid=<?=$gameid?>"><font size="2"><?=$d[homeaway]?><?=$d[index]?></font></a>
      <?} else {?>
        <?=$d[homeaway]?><?=$d[index]?>
      <?}
      ?></td><?
...
<center><font size="2"><u>Legend</u><br></font>
<p>
<table>
<tr>
<td>
...
<td>
   <img src="cop_up.gif">&nbsp;Change of Possession
</td>
</tr>
</table>
</p>
